...
def train_loop(tuner_config, config_kwargs):
    ...
    # Pass device here
    model = resume_model(args.checkpoint_dir, model, device)
    print(model)
...
def resume_model(checkpoint_dir, model, device):
    if checkpoint_dir != None:
        path = os.path.join(checkpoint_dir, "base.pt")
        # Load checkpoint with map_location to handle device mismatches
...
def test_phase(accum_iter, model, dataloaders, device, args, logger, epoch, best_epoch):
    print('Final test')
    if args.mode in ["train", "hyper_search"]:
        # Pass device here too
        model = resume_model(args.run_dir, model, device)
