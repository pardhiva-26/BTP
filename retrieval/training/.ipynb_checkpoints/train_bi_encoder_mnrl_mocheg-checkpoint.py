...
def load_data(args, ce_score_margin, num_negs_per_system):
    ...
    # Filter queries to only keep those with valid corpus IDs
    if args.media == "img":
        valid_queries = set()
        skipped_queries = 0
        ...
        filtered_positive_rel_docs = {}
        filtered_negative_rel_docs = {}
        
        for qid in needed_qids:
            if qid in positive_rel_docs and qid in negative_rel_docs:
                # Check if at least one positive and one negative exist
                valid_pos = [pid for pid in positive_rel_docs[qid] if pid in corpus_keys]
                valid_neg = [pid for pid in negative_rel_docs[qid] if pid in corpus_keys]
                
                if valid_pos and valid_neg:
                    # Store only valid corpus IDs in filtered dictionaries
                    filtered_positive_rel_docs[qid] = set(valid_pos)
                    filtered_negative_rel_docs[qid] = valid_neg
                    valid_queries.add(qid)
                else:
                    skipped_queries += 1
        
        # Replace with filtered versions
        positive_rel_docs = filtered_positive_rel_docs
        negative_rel_docs = filtered_negative_rel_docs
        needed_qids = valid_queries
        
        logging.info(f"Skipped {skipped_queries} queries due to missing images")
        logging.info(f"Valid queries: {len(needed_qids)}")
