...
class ImageDataDealer:
    ...
    def load_corpus(self, data_folder, corpus_max_size):
        ...
        print(f"Loading corpus from: {image_corpus}")
        print(f"Sample keys from relevant_document_img_list: {relevant_document_img_list[:5]}")
        
        for relevant_document_img in relevant_document_img_list:
            img_path = os.path.join(image_corpus, relevant_document_img)
            ...
            if os.path.isdir(img_path):
                for img_file in os.listdir(img_path):
                    if img_file.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
                        ...
                        # Key format might need to match qrels format
                        corpus[img_file] = full_img_path
            elif os.path.isfile(img_path):
                corpus[relevant_document_img] = img_path
        
        print(f"Total corpus size: {len(corpus)}")
        print(f"Sample corpus keys: {list(corpus.keys())[:5]}")
        return corpus
...
def gen_corpus_embedding(corpus, model):
    ...
    for corpus_id, corpus_img_path in corpus.items():
        try:
            ...
            if not os.path.exists(corpus_img_path):
                logging.warning(f"Image file not found: {corpus_img_path}")
                continue
                
            # Check if it's a directory
            if os.path.isdir(corpus_img_path):
                logging.warning(f"Path is a directory, not a file: {corpus_img_path}")
                continue
            ...
            if live_num_in_current_batch % batch_size == 0:
                img_emb = model.encode(current_image_batch, batch_size=batch_size, 
                                      convert_to_tensor=True, show_progress_bar=True)
                total_img_emb = torch.cat([total_img_emb, img_emb], 0)
                live_num_in_current_batch = 0
                current_image_batch = []
                live_num += batch_size
                print(f"Progress: {live_num}/{corpus_len}")
        except Exception as e:
            logging.warning(f"Error processing image {corpus_id}: {e}")
            continue
    
    # Process remaining images
    if current_image_batch:
        img_emb = model.encode(current_image_batch, batch_size=len(current_image_batch), 
                              convert_to_tensor=True, show_progress_bar=True)
        total_img_emb = torch.cat([total_img_emb, img_emb], 0)
...
class MSMARCOImageDataset(MSMARCODataset):
    def __init__(self, queries, corpus):
        ...
        self.missing_images = set()  # Track missing images
        
    def obtain_corpus_by_idx(self, corpus_id):
        if corpus_id not in self.corpus:
            # Log missing image once
            if corpus_id not in self.missing_images:
                self.missing_images.add(corpus_id)
                logging.warning(f"Image not found in corpus: {corpus_id}")
            
            # Return a blank/dummy image instead of crashing
        ...
        try:
            ...
            logging.warning(f"Error loading image {corpus_id}: {e}")
            return Image.new('RGB', (224, 224), color='black')
